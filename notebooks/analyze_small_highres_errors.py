"""
Generate TTA-aware error analysis for small high-resolution AutoVision models.
"""
...
from train_small_highres import SMALL_MODELS, build_model


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create small high-resolution prediction and mistake reports.")
...
def load_model(checkpoint_path: Path, device: torch.device) -> tuple[nn.Module, dict[str, Any], list[str]]:
    ...
    model_name = checkpoint.get("model_name", checkpoint_args.get("model_name", "efficientnet_b0"))
    if model_name not in SMALL_MODELS:
        raise ValueError(f"Expected one of {sorted(SMALL_MODELS)}, but checkpoint model_name is {model_name!r}.")

    model_args = SimpleNamespace(
        model_name=model_name,
        pretrained=False,
        dropout=float(checkpoint_args.get("dropout", 0.30)),
    )
